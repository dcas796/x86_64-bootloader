...
fat_result_t fat_read(const fat_file_t *file, uint32_t offset, uint32_t length, void *buffer) {
    ...
    while (length != 0) {
        ...
        uint32_t sector_offset = offset_in_cluster / file->fat->bpb.bytes_per_sector;
        uint32_t length_to_read = min(length + offset_in_cluster, bytes_per_cluster) - offset_in_cluster;
